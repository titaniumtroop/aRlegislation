...
packages <- c("aRlegislation", "tidyverse", "furrr", "parallel", "e1071", "scales", "caret", "ggplot2", "ggridges", "RColorBrewer")
...
lawmaker_models <- load_lawmaker_models(datadir = "./inst/extdata/", topics = 100:205)
...
temp_model <- temp_model <- lawmaker_models$lawmaker_model[[4]]
x <- temp_model[-1]
y <- as.numeric(x$corruption)
svmfit <- e1071::svm(corruption ~ ., data = x, kernel = "linear", cost = 500, scale = FALSE, cross = 5)

pred <- predict(svmfit, x, decision.values = TRUE)
cbind(pred, temp_model[,c(1:2)]) %>% arrange(pred)
...
set.seed(77)
grid_size <- 1040 # Should be evenly divisible by splits
splits <- 26
...
search_grid <- expand.grid(
  ...
  nu = seq(from = 0.001, to = 0.05, length.out = 50),
  cross = 1:6,
...
save(
  model_grid,
  file = paste("../data-raw/results-scores/model_grid_short ",
               gsub(":", "", Sys.time()),
               ".RData",
               sep = ""),
  envir = .GlobalEnv,
...
compute_svm <- function(model, kernel, gamma, cost, cross, degree, type, nu, train, class.weights, ...) {
  x <- model[-1] # Remove legislator name
  try({
    svmfit <- e1071::svm(
      corruption ~ ., 
      data = x, 
...
save(
  SVM_results,
  file = paste("../data-raw/results-scores/SVM_results_short_refined_", iter, " ",
               gsub(":", "", Sys.time()),
               ".RData",
               sep = ""),
  envir = .GlobalEnv,
...
save(
  corruption_SVM,
  file = paste("../data-raw/results-scores/corruption_SVM_short_", iter, " ",
               gsub(":", "", Sys.time()),
               ".RData",
               sep = ""),
  envir = .GlobalEnv,
...
ggsave(paste0("../data-raw/results-scores/images/SVM_short_refined_topics_", iter, ".png"), height = height, width = width)
...
ggsave(paste0("../data-raw/results-scores/images/SVM_short_refined_nu_", iter, ".png"), height = height, width = width)
...
ggsave(paste0("../data-raw/results-scores/images/SVM_short_refined_cross_", iter, ".png"), height = height, width = width)
...
ggsave(paste0("../data-raw/results-scores/images/SVM_short_refined_split_", iter, ".png"), height = height, width = width)
...
ggsave(paste0("../data-raw/results-scores/images/SVM_short_refined_weight_ratio_", iter, ".png"), height = height, width = width)
